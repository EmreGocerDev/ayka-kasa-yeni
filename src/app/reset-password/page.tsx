...
  useEffect(() => {
    // URL'den `access_token`'ı ve `refresh_token`'ı al
    const accessToken = searchParams.get('access_token');
    const refreshToken = searchParams.get('refresh_token');

    // Eğer token'lar varsa, oturumu kendimiz ayarlayalım
    if (accessToken && refreshToken) {
        // Oturumu manuel olarak ayarlayın
        supabase.auth.setSession({
            access_token: accessToken,
            refresh_token: refreshToken
        }).then(({ error }) => {
            if (error) {
                setResult({ error: 'Oturum oluşturulurken bir hata oluştu. Lütfen tekrar deneyin.' });
            } else {
                // Oturum başarıyla ayarlandı, şimdi URL'yi temizle
